...
class AnalysisDaily implements FromView
{

	public function __construct(string $study_group_id,string $kd_id,string $class_grade_id)
    {
        $this->kd_id = $kd_id;
   ...
   use Exportable;
	...
	public function view(): View
	{
        ...
        ->where('kd_id',$this->kd_id)
        ->join('m_course as mc','mc.course_id','z.course_id')->first();
        ...
        if(count($data_bobot) == 0)
        {
		 ...
		 $data = ['data_siswa'=>$data_siswa,'data_bobot'=>$data_bobot,'kd'=>$kd,'walas'=>$walas];
		return view('excel.analysis_daily', [
           	 	'data' => $data
        ]);
