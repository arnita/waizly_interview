   	...
   	<tbody>
        ...
        @foreach($data['data_siswa'] as $row)
            <tr>   
                ...
                @foreach($data['course_setting'] as $cs)        
	                @php
		    			$k3 = \App\Models\Raport_pas::where('course_id',$cs->course_id)
		    			...
		    			elseif(!empty($k3->total_pengetahuan) && !empty($k3->total_keterampilan))
		    			{
		    	...
		    	<!-- ini untuk kolom rata rata -->
		    		@php
                     $k3_tot = \DB::table('raport_pas')->select(\DB::raw('sum(total_pengetahuan) /  count(course_id ) as tot_peng,sum(total_pengetahuan) as sum_peng,count(course_id) as count_course'))->where('student_id',$row->student_id)->where('step_study_id',stepStudyActive())->first();
		    		@endphp
		    		@php
                     $k3_tot_ket = \DB::table('raport_pas')->select(\DB::raw('sum(total_keterampilan) /  count(course_id ) as tot_ket,sum(total_keterampilan) as sum_ket,count(course_id) as count_course'))->where('student_id',$row->student_id)->where('step_study_id',stepStudyActive())->first();
		    		@endphp
		    	<td  style="text-align:center">
		    		{{ round($k3_tot->tot_peng)}}
		    	</td>
		    	<td  style="text-align:center">
		    		{{ round($k3_tot_ket->tot_ket)}}
		    	</td>
		    	<td style="text-align:center">
		    		{{ round ( ($k3_tot->sum_peng + $k3_tot_ket->sum_ket) / 2 ) }}
