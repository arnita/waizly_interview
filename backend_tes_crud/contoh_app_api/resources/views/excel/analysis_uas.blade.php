...
<table ref="table" id="loremTable" summary="Input analisis" class="table table-bordered">
	<thead>
        ...
        <tr>
            ...
            <th style="font-weight:bold; ">:</th>
            <th style="font-weight:bold; ">  {{  (!empty($data['data_bobot'][0])) ? $data['data_bobot'][0]->course_name  : "" }}  </th>
        </tr>
		...
		@foreach($data['data_siswa'] as $row)
		    <tr>   
		        ...
		        <td>{{$row->student_name}}</td>
			        @foreach($data['data_bobot'] as $r)
			    		@php	
							...
							->where('kd_id',$r->kd_id)->where('student_id',$row->student_id)->where('number_question',$r->number_question)->where('input_analysis_type','uas')->first();
	    		...
	    		@php	
					...
					->where('step_study_id',stepStudyActive())
					->where('student_id',$row->student_id)->where('input_analysis_type','uas')->whereIn('kd_id',$arr_val)->first();
		        @endphp
		    <!--     @if($tot == 0)
		        	<td></td>
		        @else
