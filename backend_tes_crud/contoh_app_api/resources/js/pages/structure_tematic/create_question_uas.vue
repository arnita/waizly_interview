        ...
        <div class="col-sm-12">
            ...
            <div class="card">
                <div class="card-header">
                    <h5 class="card-header-text"> Buat Soal PAS TEMATIK </h5>
                </div>
...
<script>
    ...
    export default{
        ...
        created(){
            this.get_uas_tema(this.id).then((resp)=>{
        ...
        computed:{
            ...mapState('application', {
                uas_tema:state=>state.uas_tema,
            }),
        },
       
        methods:{
            ...mapActions('application',['get_uas_tema']),
            insert_number(e,id,kd_id){
                $axios.post(`/insert_number_uas`,{'number_question':e.target.value,'id':id,'kd_id':kd_id,'structure_tematic_id':this.structure_tematic_id})
                    .then(function(resp){
            ...
            insert_quality(e,id,kd_id){
                $axios.post(`/insert_quality_uas`,{'quality_question':e.target.value,'id':id,'kd_id':kd_id})
                    .then(function(resp){
                        // swal('Success','Simpan data berhasil','success');
                })   
            },
            async cek_number(){
                var app = this;                
                await $axios.get('/cek_number?type_question=uas&id='+app.id+'&structure_tematic_id='+app.structure_tematic_id)
                .then(function(resp){
